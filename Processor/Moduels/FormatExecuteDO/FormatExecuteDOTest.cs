...
namespace SFTemplateGenerator.Processor.Moduels.FormatExecuteDO
{
    public class FormatExecuteDOTest : IFormatExecuteDOTest
    {
        ...
        {
            Logger.Info($"开出传动测试");
            var boards = TargetDevice.Boards.Where(B => DOBORAD_REGEX.Any(R=>R.IsMatch(B.Desc))|| SIGNALBORAD_REGEX.IsMatch(B.Desc) || POWERBORAD_REGEX.Any(R=>R.IsMatch(B.Desc))).ToList();
